...
/**
 * IncendioCorretaScene — prevenção correta de incêndio
 * Duração: 155 frames com fadeout automático
 */
export const IncendioCorretaScene: React.FC<{totalDuration?: number}> = ({totalDuration = 155}) => {
	...
	const fadeOut = interpolate(frame, [totalDuration - 24, totalDuration - 4], [1, 0], {
		extrapolateLeft: 'clamp', extrapolateRight: 'clamp',
	});
	...
	return (
		<AbsoluteFill style={{opacity, fontFamily: FONT}}>
			...
			<div style={{
				...
				<Img
					src={staticFile('incendio-correto.png')}
					style={{
						width: '100%',
						height: '100%',
						objectFit: 'cover',
			...
			<svg width="1920" height="1080" viewBox="0 0 1920 1080"
				...
				<defs>
					<filter id="icCheckShadow" x="-40%" y="-40%" width="180%" height="180%">
				...
				{checkProgress > 0 && (
					<g transform={`translate(${FRAME_X + FRAME_W - 80}, ${FRAME_Y - 28})`}
						...
						<circle cx="40" cy="40" r="38" fill={COLORS.green} filter="url(#icCheckShadow)" />
						<circle cx="40" cy="40" r="38" fill={COLORS.green} />
						<path d="M 18 40 L 33 56 L 62 22"
				...
				<div style={{
					...
					<div style={{
						...
						<span style={{
							...
							Sinalização adequada salva vidas.
						</span>
					</div>
				</div>
			)}
